...
volatile void *_ramQPrevTail; //tail pointer
...
volatile uint16_t _packetSz;

volatile bool _isRamQLock;

enum ramqState_t ramqState;
enum ramqState_t ramqPrevState;

//This function begin ramq
void ramQSet(void *bufPtr, uint16_t packetSz, uint8_t totalPacket)
{
  _baseAddr = bufPtr;
  ...
  _isRamQLock = false;
  ramqState = RESET;
  ramqPrevState = ramqState;

}

// return to last tail value;
void *ramqFallBack()
{
	if(ramqPrevState != RESET)
	{
		_ramQTail = _ramQPrevTail;
		return _ramQPrevTail;
	}
	else
	{
		return NULL;
